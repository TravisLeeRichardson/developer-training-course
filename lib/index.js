...
const {core, utils, values} = import("@ckb-lumos/base");
const {CellCollector} = import("@ckb-lumos/ckb-indexer");
//const {computeScriptHash} = utils;
import { computeScriptHash } from "@ckb-lumos/base/lib/utils.js"; //hmmm. this works, but should I be doing it this way?
import {secp256k1Blake160} from "@ckb-lumos/common-scripts";
import {locateCellDep, sealTransaction} from "@ckb-lumos/helpers";
//const {ScriptValue} = values;
import {ScriptValue} from "@ckb-lumos/base/lib/values.js"; //hmmm. thjis works, but should I be doing it this way?
import { FileSystemModule } from "bufferutility";

import { commons } from "@ckb-lumos/lumos" // not sure on this one.  referenced in https://lumos-website.vercel.app/migrations/migrate-from-ckb-sdk-core example I found.

// Got this error on the below line: 
//SyntaxError: Named export 'add' not found. The requested module 'lodash' is a CommonJS module, which may not support all module.exports as named exports.
...
const {normalizers} = import ("@ckb-lumos/toolkit");
//const {Reader} = import ("@ckb-lumos/toolkit/lib/reader.js");
...
export function addDefaultWitnessPlaceholders(transaction)
{
	...
	for(const input of transaction.inputs)
	{
		...
		const newWitnessArgs = { lock: witness };
...
export function checkTX_FEE(transaction)
{
 ...
 * @returns {Object} An object with the inputCells[] found and the inputCapacity contained within the provided Cells.  
 */
export async function collectCapacityWithType(indexer, lockScript, typeScript, capacityRequired)
{
...
export function describeTransaction(transaction, options)
{
	...
	for(const dep of transaction.cellDeps)
	{
		...
		{
			depType: dep.depType,
			outPoint: dep.outPoint.tx_hash + "-" + dep.outPoint.index
		};
		obj.deps.push(cell);
	}
	...
	for(const input of transaction.inputs)
	{
		...
		let cell =
		{
			...
			outPoint: input.outPoint.tx_hash + "-" + input.outPoint.index,
			data: input.data
		};
...
export async function getLiveCell(NODE_URL, outPoint, returnData = false)
{
	...
	const res = await rpc.getLiveCell({txHash: outPoint.tx_hash, index: outPoint.index}, returnData);
...
export function signMessage(PRIVATE_KEY, message)
{
	...
	const {signature, recid} = secp256k1.ecdsaSign(messageArray, pkArray); // error happening here.
...
//module.exports = {addDefaultCellDeps};
