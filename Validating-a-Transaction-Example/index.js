...
// This is the IC3Type RISC-V binary.
const DATA_FILE_1 = "../files/ic3type";
const DATA_FILE_HASH_1 = ckbHash(hexToArrayBuffer(readFileToHexStringSync(DATA_FILE_1).hexString)).serializeJson(); // Blake2b hash of the IC3Type binary.
...
async function deployCode(indexer)
{
	...
	// Return the out point for the IC3Type binary so it can be used in the next transaction.
	const outPoint =
	{
		txHash: txid,
		index: "0x0"
...
async function createCells(indexer, ic3typeCodeOutPoint)
{
	// Create a transaction skeleton.
	let transaction = TransactionSkeleton();
	...
	// Determine the capacity of all input and output cells.
	const inputCapacity = transaction.inputs.toArray().reduce((a, c)=>a+hexToInt(c.cellOutput.capacity), 0n);
	const outputCapacity = transaction.outputs.toArray().reduce((a, c)=>a+hexToInt(c.cellOutput.capacity), 0n);
...
async function consumeCells(indexer, ic3typeCodeOutPoint)
{
	// Create a transaction skeleton.
	let transaction = TransactionSkeleton();
...
async function main()
{
	...
	// Create a cell that contains the IC3Type binary.
	const ic3typeCodeOutPoint = await deployCode(indexer);
	await indexerReady(indexer);

	// Create a cell that uses the IC3Type script.
	await createCells(indexer, ic3typeCodeOutPoint);
	await indexerReady(indexer);

	// Consume the cell using the IC3Type script.
	await consumeCells(indexer, ic3typeCodeOutPoint);
	await indexerReady(indexer);
