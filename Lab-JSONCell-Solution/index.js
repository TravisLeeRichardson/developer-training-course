...
async function createCells(indexer, scriptCodeOutPoint)
{
	...
	for(const jsonData1 of ["Hello World!", ["Foo", "Bar"], {"Lorem": "Ipsum"}])
	{
		const jsonString1 = JSON.stringify(jsonData1);
		const data1 = stringToHex(jsonString1);
		const outputCapacity1 = ckbytesToShannons(94n + BigInt(jsonString1.length));
		const output1 = {cellOutput: {capacity: intToHex(outputCapacity1), lock: lockScript1, type: typeScript1}, data: data1};
		transaction = transaction.update("outputs", (i)=>i.push(output1));
	}

	// Determine the capacity of all output cells.
	const outputCapacity1 = transaction.inputs.toArray().reduce((a, c)=>a+hexToInt(c.cellOutput.capacity), 0n);
...
async function main()
{
	...
	// Create a cell that uses the script code binary as a lock script.
	await createCells(indexer, scriptCodeOutPoint);
	await indexerReady(indexer);

	// Consume the cell we just created.
