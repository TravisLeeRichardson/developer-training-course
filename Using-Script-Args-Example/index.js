...
// This is the DataCap RISC-V binary.
const DATA_FILE_1 = "../files/datacap";
const DATA_FILE_HASH_1 = ckbHash(hexToArrayBuffer(readFileToHexStringSync(DATA_FILE_1).hexString)).serializeJson(); // Blake2b hash of the DataCap binary.
...
async function deployCode(indexer)
{
	...
	// Return the out point for the DataCap binary so it can be used in the next transaction.
	const outPoint =
	{
		txHash: txid,
		index: "0x0"
...
async function createCells(indexer, dataCapCodeOutPoint)
{
	...
	transaction = addDefaultCellDeps(transaction);
	const cellDep = {depType: "code", outPoint: dataCapCodeOutPoint};
