...
async function createCells(indexer, scriptCodeOutPoint)
{
	...
	// Create a cell.
	for(const counterValues of [[0n, 0n], [42n, 42n], [9_000n, 9_000n]])
	{
...
async function updateCells(indexer, scriptCodeOutPoint, cellOutPoints)
{
	// Create a transaction skeleton.
	let transaction = TransactionSkeleton();
	...
	for(const cellOutPoint of cellOutPoints)
	{
		const input = await getLiveCell(NODE_URL, cellOutPoint, true);
		transaction = transaction.update("inputs", (i)=>i.push(input));
	}

	// Add the updated cell to the transaction.
	for(let i = 0; i < cellOutPoints.length; i++)
	{
		const outputCapacity1 = ckbytesToShannons(110n);
		const lockScript1 = addressToScript(ADDRESS_1);
		const typeScript1 =
		...
		const counterValue2 = u64LeHexBytesToInt("0x" + transaction.inputs.get(i).data.substr(18, 16));
...
async function main()
{
	...
	const cellOutPoints = await createCells(indexer, scriptCodeOutPoint);
	await indexerReady(indexer);

	// Update the cells created in the last transaction.
	await updateCells(indexer, scriptCodeOutPoint, cellOutPoints);
	await indexerReady(indexer);
