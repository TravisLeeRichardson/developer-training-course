...
async function createCells(indexer, codeOutPoint)
{
	// Create a transaction skeleton.
	let transaction = TransactionSkeleton();
	...
	// Add the cell deps.
	transaction = addDefaultCellDeps(transaction);
	const cellDep = {depType: "code", outPoint: codeOutPoint};
...
async function updateCells(indexer, codeOutPoint, cellOutPoints)
{
	// Create a transaction skeleton.
	let transaction = TransactionSkeleton();
	...
	const input = await getLiveCell(NODE_URL, cellOutPoints, true);
	...
	// Add the updated cells to the transaction.
	const outputCapacity1 = ckbytesToShannons(110n);
	const lockScript1 = addressToScript(ADDRESS_1);
	const typeScript1 =
	{
...
async function main()
{
	...
	const codeOutPoint = await deployCode(indexer);
	await indexerReady(indexer);

	// Create cells that uses the binary that was just deployed.
	const cellOutPoints = await createCells(indexer, codeOutPoint);
	await indexerReady(indexer);

	// Update the cells using the binary from the last transaction.
	await updateCells(indexer, codeOutPoint, cellOutPoints);
	await indexerReady(indexer);
