...
async function createCells(indexer, doubleCounterCodeOutPoint)
{
	...
	// Add the cell deps.
	transaction = addDefaultCellDeps(transaction);
	const cellDep = {depType: "code", outPoint: doubleCounterCodeOutPoint};
	...
	// Return the out point of the cell so it can be used in the next transaction.
	const outPoint =
	{
		txHash: txid,
		index: "0x0"
...
async function updateCells(indexer, doubleCounterCodeOutPoint, doubleCounterCellOutPoint)
{
	...
	// Add the cell to the transaction.
...
async function main()
{
	...
	// Create a cell that contains the binary.
	const doubleCounterCodeOutPoint = await deployCode(indexer);
	await indexerReady(indexer);

	// Create cells that use the binary that was just deployed.
