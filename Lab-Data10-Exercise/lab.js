...
async function validateLabCreate(skeleton)
{
	...
	for(let i = 0; i < 3; i++)
	{
		if(BigInt(tx.outputs[i].cellOutput.capacity) !== ckbytesToShannons(104n))
			throw new Error(`This lab requires output ${i} to have a capacity of 104 CKBytes.`);
		...
		if(new ScriptValue(tx.outputs[i].cellOutput.type).hash() !== "0x68b82f41985e75fea6fdbb1eb79d8d98f1c7db1d14663440eb5749a3eb3bfa82")
			throw new Error(`This lab requires output ${i} to use the Data10 type script.`);

		const message = ["HelloWorld", "Foo Bar", "LoremIpsum"][i];
		if(tx.outputs[i].data !== stringToHex(message))
			throw new Error(`This lab requires output ${i} to contain the data "${message}".`);
	}
...
async function validateLabConsume(skeleton)
{
	...
	for(let i = 0; i < 3; i++)
	{
		if(BigInt(tx.inputs[i].cellOutput.capacity) !== ckbytesToShannons(104n))
			throw new Error(`This lab requires input ${i} to have a capacity of 104 CKBytes.`);
		...
		if(new ScriptValue(tx.inputs[i].cellOutput.type).hash() !== "0x68b82f41985e75fea6fdbb1eb79d8d98f1c7db1d14663440eb5749a3eb3bfa82")
			throw new Error(`This lab requires input ${i} to use the Data10 type script.`);

		const message = ["HelloWorld", "Foo Bar", "LoremIpsum"][i];
		if(tx.inputs[i].data !== stringToHex(message))
			throw new Error(`This lab requires input ${i} to contain the data "${message}".`);
	}
